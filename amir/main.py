...
#    kl_loss_4 = - 0.5 * K.sum(1 + _z_log_var_2_2 - K.square(_z_mean_2_2) - K.exp(_z_log_var_2_2), axis=-1)
    ...
    y_loss_1 = 10000 * objectives.categorical_crossentropy(yy_1, _y_decoded_1)
    y_loss_2 = 100000 * objectives.categorical_crossentropy(yy_2, _y_decoded_2)
...
# model_weights = pickle.load(open('proposed_method_' + str(latent_dim_y) + 'd_trained_on_' + dataset_name, 'rb'))
# model.set_weights(model_weights)
...
pickle.dump((model_weights), open('proposed_method_' + str(latent_dim_y) + 'd_trained_on_' + dataset_name, 'wb'))
